...
HYDRA_CHOICES     = ["Easy", "Normal", "Hard", "Brutal", "NM", "UNM"]
CHIMERA_CHOICES   = ["Easy", "Normal", "Hard", "Brutal", "NM", "UNM"]
TRI_CHOICES       = ["—", "Yes", "No"]
ROSTER_CHOICES    = ["All", "Open only", "Full only"]
...
class ClanMatchView(discord.ui.View):
    ...
    class CBSelect(discord.ui.Select):
        def __init__(self, view:"ClanMatchView"):
            ...
            super().__init__(placeholder="Clan Boss (optional)", min_values=0, max_values=1, options=options, row=0)
    ...
    class HydraSelect(discord.ui.Select):
        def __init__(self, view:"ClanMatchView"):
            ...
            super().__init__(placeholder="Hydra (optional)", min_values=0, max_values=1, options=options, row=0)
        async def callback(self, itx: discord.Interaction):
    ...
    class ChimeraSelect(discord.ui.Select):
        def __init__(self, view:"ClanMatchView"):
            ...
            super().__init__(placeholder="Chimera (optional)", min_values=0, max_values=1, options=options, row=0)
        async def callback(self, itx: discord.Interaction):
    ...
    class PlaystyleInput(discord.ui.Select):
        def __init__(self, view:"ClanMatchView"):
            self._parent = view
            options = [discord.SelectOption(label=o, value=o) for o in ["", "Relaxed", "Semi-competitive", "Competitive", "Any"]]
            super().__init__(placeholder="Playstyle (optional)", min_values=0, max_values=1, options=options, row=1)
        async def callback(self, itx: discord.Interaction):
    ...
    class CvCButtons(discord.ui.Item):
        def __init__(self, view:"ClanMatchView"):
            ...
            self._parent = view
        async def callback(self, interaction): pass  # not used
    ...
    class RosterButtons(discord.ui.Item):
        ...
        async def roster_full(self, itx: discord.Interaction, _btn: discord.ui.Button):
            self._parent.roster_mode = 0
            await itx.response.defer()
    class SearchBtn(discord.ui.Button):
        def __init__(self, view:"ClanMatchView"):
            super().__init__(label="Search Clans", style=discord.ButtonStyle.primary, row=4)
            self._parent = view
        async def callback(self, itx: discord.Interaction):
            await self._parent.search(itx, self)
    class ResetBtn(discord.ui.Button):
        def __init__(self, view:"ClanMatchView"):
            super().__init__(label="Reset", style=discord.ButtonStyle.secondary, row=4)
            self._parent = view
        async def callback(self, itx: discord.Interaction):
            v = self._parent
    ...
    def _sync_visuals(self):
        # (kept minimal; buttons/selects already reflect state upon next interaction)
        pass
    ...
    async def _ghost(self, itx: discord.Interaction, _btn: discord.ui.Button):
        # never shown; placeholder for layout stability in some clients
        try:
            await itx.response.defer()
    ...
    async def search(self, itx: discord.Interaction, _btn: discord.ui.Button):
        ...
        for row in rows[1:]:
            try:
                if is_header_row(row):
                    continue
                if not row_matches(row, cb, hydra, chim, cvc, siege, style):
                    continue
                # roster filter
                if roster is not None:
                    open_spots = parse_spots(row[COL_E_SPOTS] or "")
                    if roster == 1 and open_spots <= 0:
                        continue
                    if roster == 0 and open_spots > 0:
                        continue
                matches.append(row)
            except Exception:
                continue
...
async def on_raw_reaction_add(payload: discord.RawReactionActionEvent):
    ...
    try:
        ...
        if "Profile" in (msg.embeds[0].title or ""):
            # flip back to entry criteria (classic)
...
async def clanmatch_cmd(ctx: commands.Context, *, extra: str | None = None):
    if extra and extra.strip():
        ...
        return
    if not _cooldown_ok(ctx.author.id):
        return
